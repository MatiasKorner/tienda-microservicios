...
public class PedidoController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<PedidoResponse> findById(@PathVariable @NonNull Long id) {
        return ResponseEntity.ok(pedidoService.findById(id));
    }

    @GetMapping("/usuario/{idUsuario}")
    public ResponseEntity<List<PedidoResponse>> findByUsuarioId(@PathVariable String email) {
        return ResponseEntity.ok(pedidoService.findByUsuarioEmail(email));
    }
